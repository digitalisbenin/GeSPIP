    ...
    <div class="content-wrapper">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title text-uppercase ">Activités Périodiques </h3>
            </div>
        </div>
        ...
        <div class="card-body">
            ...
            <input type="date" class="form-control ml-3 "id="endDate">
              <button onclick="imprimerTable()" class="btn btn-primary text-white d-flex ml-2">
...
<td class="category-date" >{{ \Carbon\Carbon::parse($task->date_achev_pro)->format('d/m/Y') }}</td>
{{--  <td>
...
$(document).ready(function() {
    $("#searchInput, #searchInputs, #startDate, #endDate").on("keyup change", function() {
        let value = $("#searchInput").val().toLowerCase().trim();
        let value2 = $("#searchInputs").val().toLowerCase().trim();
        let startDate = $("#startDate").val();
        let endDate = $("#endDate").val();

        console.log("Valeurs de recherche :", value, value2, startDate, endDate);
        ...
        $("#categoryTable tr").each(function() {
            ...
            let categoryDate = $(this).find(".category-date").text().trim(); // Date affichée dans le tableau

            console.log("Row Date (original):", categoryDate);

            let dateValid = true;
            if (startDate && endDate) {
                let dateParts = categoryDate.split("/"); // Séparer JJ/MM/AAAA
                if (dateParts.length === 3) {
                    let day = parseInt(dateParts[0]);
                    let month = parseInt(dateParts[1]) - 1; // Les mois commencent à 0 en JS
                    let year = parseInt(dateParts[2]);

                    let rowDate = new Date(year, month, day).getTime(); // Convertir en timestamp
                    let start = new Date(startDate).getTime();
                    let end = new Date(endDate).getTime();

                    // Vérifier si la conversion est valide avant comparaison
                    if (!isNaN(rowDate) && !isNaN(start) && !isNaN(end)) {
                        dateValid = rowDate >= start && rowDate <= end;
                    } else {
                        dateValid = false;
                    }
                } else {
                    dateValid = false; // Format incorrect
                }
            }

            // Vérification des critères de recherche
            let matches = title.startsWith(value) &&
                          (description.startsWith(value2) || value2 === "") &&
                          dateValid;

            console.log("Row Date (converted):", categoryDate);
...
$(document).ready(function() {
    ...
    {{--  <script>
$(document).ready(function() {
    $("#searchInput, #searchInputs, #searchInpute"#startDate, #endDate").on("keyup", function() {
